    ...
    def get_all_container_ips(self) -> Dict[str, str]:
        ...
        try:
            ...
            for container in running_containers:
                ...
                # Skip temporary Docker run containers (e.g., docker-infradmin-run-xxxx)
                # These are created by cron jobs and should not be in DNS
                if 'docker-' in container_name and '-run-' in container_name:
                    self.o_logger.debug(f"Skipping temporary container: {container_name}")
                    continue
    ...
    def sync_containers_to_dns(self, container_ips: Dict[str, str]) -> bool:
        ...
        try:
            ...
            for container_name, ip_address in container_ips.items():
                # Add container to both forward and reverse zones via dynamic updates
            ...
            self.o_logger.info(f"DNS sync complete via dynamic updates: {added_count} containers processed")
            return True
...
"""
    ...
    def sync_dns_records(self) -> bool:
        """Synchronize DNS records with running containers (DHCP mode only)"""
        try:
            ...
            # Update reverse zone file (requires reload since it's file-based)
            ...
            # (No reload needed - dynamic updates are immediate)
            if smart_sync:
...
def main():
    ...
    if args.list:
        ...
        print("\nMode: DHCP-Safe Mode with Smart Sync")
        ...
        if smart_sync:
            ...
            print("- Manual container addition required (smart sync disabled)")
